...
export default function CapturePage() {
  ...
  const loadWorkspace = useCallback(() => {
    Promise.all([
      ...
      .then(async ([wsRes, areaRes]) => {
        ...
        if (ws) localStorage.setItem("knowledgecast_workspace_id", ws.id);
      })
      .catch(() => {
  ...
  return (
    <main className="px-8 py-8">
      ...
      {workspace ? (
        <>
          <CaptureInput userId={userId} workspaceId={workspace.id} onDone={handleCaptureDone} />
          <FragmentList key={refreshKey} userId={userId} workspaceId={workspace.id} />
        </>
      ) : (

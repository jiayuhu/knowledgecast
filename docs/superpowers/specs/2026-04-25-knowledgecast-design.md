...
**已完成（2026-05-05）：**
- 版本历史 + 回退（`docs/superpowers/specs/2026-05-05-iteration-enhancements-design.md`）
- 素材覆盖分析（生成前数量提示 + 生成后 AI 语义缺口标注）
- 轻量文本编辑（标题、要点、讲者备注内联编辑，结构变更走 AI）
- 自定义框架模板
...
- AI 将当前工作集的碎片素材按框架组织为幻灯片；未手动勾选素材时，只使用当前工作集素材
...
- 发布页展示当前工作集的培训页列表，支持复制链接和预览
- 空状态：引导「去整理页生成」

### 6.3a 内容门禁

- 工作集无素材时，访问整理页自动重定向到采集页（提示：先采集素材）
- 无培训页时，访问发布页自动重定向到整理页
- 防止在无内容时进入生成/发布流程
...
- `TrainingPage`：生成的培训页面，归属于一个工作集，包含框架 ID、幻灯片 JSON、时长、版本号

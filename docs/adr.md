...
**决策**: 当用户提交 `text` 或 `markdown` 类型素材时，扫描内容中的 `https://` URL，为每个 URL 自动创建独立的 `url` 类型子素材。原始素材保留不变（含用户的 curation 上下文和链接描述），子素材为获取到的正文。

**理由**:
- 用户常粘贴含多个参考链接的笔记（如「两篇文章值得看：url1 url2」），手动逐个粘贴链接效率低
- 原始笔记和获取到的正文互补：笔记提供「为什么选这篇、用在哪」，正文提供「具体讲什么」
- `extractUrls()` 去重 + 正则提取，实现简单 10 行代码

**权衡**: 子素材获取失败不影响原始素材和兄弟子素材。子素材标题可能需要手动补充（原始笔记的链接描述未自动同步到子素材标题）。后续可优化为：将链接描述文本自动作为子素材标题。

**实现**: `src/server/ingest/normalize.ts` 的 `extractUrls()` + `src/server/ingest/storage.ts` 的 `storeKnowledgeInput()` 内循环创建。
...
## ADR-011: 素材正文区使用 Markdown 渲染

**日期**: 2026-05-04

**决策**: 素材列表中的正文从纯文本 `<pre>` 改为 Markdown 渲染（`marked` 库），图片直接显示，折叠态显示第一张缩略图。

**理由**:
- URL 获取到的正文是 Markdown 格式，纯文本显示体验差（用户看到 `## 标题`、`![alt](url)` 等语法噪音）
- 图片是培训材料的重要素材，折叠态缩略图增加辨识度，展开态直接可见
- `marked` 零依赖、轻量（~60KB），渲染前转义原始 HTML 防止 XSS

**权衡**: 展开大量图文内容时的渲染性能。当前素材列表规模（<50 条）可忽略；后续分页加载可加虚拟滚动。
